...
save_root = '/home/hu5471ch/projects/BCDI_test/NoDistortion/'
save_paths = ['ref', 'smear']
...
for i in range(len(save_paths)):
    ...
    if save_paths[i] == 'smear':
        ...
        print(smear_frames[0].shape)
        np.savez_compressed(save_path+'/sim_smear_2.npz', offsets=theta, frames=smear_frames, particle=v.data)
        
        data_sum = np.log10(np.sum(smear_frames, axis=(1,2)))
        plt.plot(theta_smear, data_sum)
    else:
        np.savez_compressed(save_path+'/sim_ref_41.npz', offsets=theta, frames=frames, particle=v.data)
        
        data_sum = np.log10(np.sum(frames, axis=(1,2)))
        plt.plot(theta, data_sum, 'r')

data_sum = np.log10(np.sum(frames, axis=(1,2)))


plt.legend(['Ref.', 'Smear'])
