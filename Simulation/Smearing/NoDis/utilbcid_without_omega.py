...
This is the sample version of Alex EMC library. In this version, we neglect the omega dependence.
...
def inner_Mbuild(j, data, Pjk):
    ...
    for k in range(Nk):
        ...
        mask = (data_k >= 0)
        norm[:] += mask * Pjk[j, k]
        Wj[:] = Wj + mask * Pjk[j, k] * data_k
    Wj[:] = Wj / (norm + 1e-20)
    return Wj


def build_model(data, Pjk, nproc=4):
    """
    ...
    inner_ = partial(inner_Mbuild, data=data, Pjk=Pjk)
    W = np.array(pool.map(inner_, range(Pjk.shape[0])))
    pool.terminate()
    return W


def inner_Mcalc(j, W, data, Nk):
    """
    Broken out inner loops of the Rjlk calculation, for paralllelization.
    """
    logRjk = np.empty((Nk), dtype=np.float64)
    for k in range(Nk):
        data_k = data[k]
        mask = (data_k >= 0)
        logRjk[k] = np.sum(mask * (data_k * np.log(W[j] + 1e-20) - W[j]))
    return logRjk


def M(W, data, beta=1.0, force_continuity=6, nproc=4, find_direction=True):
    """
    ...
    t1 = time.time()
    pool = multiprocessing.Pool(nproc)
    inner_ = partial(inner_Mcalc, W=W, data=data, Nk=Nk)
    logRjk = np.array(pool.map(inner_, range(Nj)))
    ...
    W = build_model(data, Pjk, nproc=nproc)
    ...
    if find_direction:
        ...
        vert_com = np.sum(vert_inds * W, axis=(1, 2)) / (np.sum(W, axis=(1, 2)) + 1e-16)
...
class ProgressPlot(object):
    def __init__(self):
        plt.ion()
        self.fig, self.ax = plt.subplots(ncols=4, figsize=(12, 3))
        plt.pause(.1)

    def update(self, W, Pjk, errors, vmax=0.1):
        ax = self.ax
        Nj = Pjk.shape[0]
        N =  W.shape[-1]
        [a.clear() for a in ax]
        ax[0].imshow(W[:, N//2+1, :], vmax=W[:, N//2+1, :].max() * vmax)
