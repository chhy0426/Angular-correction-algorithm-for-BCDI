...
if __name__ == '__main__':
    ...
    simfiles = [f for f in os.listdir('./Distortion/smear') if ('sim_' in f) and (f.endswith('.npz'))]
    for filename in simfiles:
        ...
        for k in range(len(co)):
            ...
            data = np.load('./Distortion/smear/' + filename)['frames']
            strg = filename.split('.npz')[0].split('sim_')[1] + 'Dmax%d'%Dmax_h + 'FOURTIMES7'
            ...
            # dq3 = Q3 / Nj
            #dq3_co = 1
            dq3 = 0.8 / data.shape[0] / 180 * np.pi * G /dq3_co #need to /2
            Q3 = dq3 *  (Nj-1)
            ...
            f.write('fudge_h=' + str(fudge_h) + '  ' + 'force_continuity=' + '%d if i < 50 else 10  '%fc_h + 'Dmax=%d '%(Dmax*1e9) + 'fudge_step=' + '2**(1/2)' + '  ' + 'dq3_co=%.02f' %dq3_co + '  ' + 'Nj_max=%d'%Nj_max + '\n')
            f.write('\n')
            f.close()
            ...
            for i in range(iter_num):
                ...
                if i < iter_num-1:
                    W, Pjk, timing = M(W, data,  beta=fudge,
                                        #force_continuity=(fc_h if i < 50 else 10), nproc=20,
                                        force_continuity=(fc_h), nproc=20,
                                        find_direction=(i > 10))
            ...
            np.savez('./Distortion/smear/assem_%s.npz' %strg, W=W, W2=W2, Pjk=Pjk, Q=(Q3, Q12, Q12))
            plt.savefig('./Distortion/smear/process_%s.png' %strg)
